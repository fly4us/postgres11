...
RUN set -x \
        && apt-get update \
	&& apt-get install -y --no-install-recommends ca-certificates wget tar make gcc libssl-dev zlib1g-dev postgresql-server-dev-11 \
	...
	&& cd .. \
	&& rm -rf pg_repack-* \
	&& tar zxvf pgpool.tar.gz && rm pgpool.tar.gz \
	&& cd pgpool-II-* \
	&& ./configure \
	&& make \
	&& make install \
	...
	&& make install \
        && cd ../../../.. \
        && rm -rf pgpool-II-* \
	&& apt-get purge -y --auto-remove ca-certificates wget tar make gcc libssl-dev zlib1g-dev postgresql-server-dev-11 \
	&& rm -rf /var/lib/apt/lists/*
